...
from .serializers import SensorReadingsSerializer, SensorExceptionDataSerializer
...
class SensorExceptionCreateView(APIView):
    def post(self, request, *args, **kwargs):
        serializer = SensorExceptionDataSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Data created successfully!", "data": serializer.data}, status=status.HTTP_201_CREATED)
        return Response({"message": "Invalid data", "errors": serializer.errors}, status=status.HTTP_400_BAD_REQUEST)